...
const Chat = () => {
  ...
  useEffect(() => {
    async function fetchHistorico() {
      try {
        ...
        const historicoFormatado = processarHistoricoDoServidor(data);
  ...
  return (
    <div className="w-[1280px] mx-auto flex flex-col items-center h-screen py-10">
      ...
      <form onSubmit={handleSubmit} className='w-full max-w-2xl flex gap-2'>
        <input 
          ...
          className='flex-1 border-2 border-blue-600 rounded-xl outline-none px-4 py-2' 
          type="text" 
          value={input} 
        ...
        <button 
          disabled={loading}
          className='bg-blue-600 text-white px-6 py-2 rounded-xl hover:bg-blue-700 transition disabled:bg-blue-300'
          type="submit"
        >
          Enviar
        </button>
